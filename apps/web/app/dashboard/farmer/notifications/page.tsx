    ...
    <div className="space-y-8">
      <div>
        ...
        <p className="text-muted-foreground mt-2">
          Stay updated on bid offers and important platform alerts
        </p>
      </div>
      ...
      {loading ? (
        ...
        <div className="space-y-4">
          {notifications.map((notif) => (
            <Card 
              key={notif.id} 
              className={notif.read_status ? 'opacity-75' : 'border-primary'}
            >
              ...
              <CardContent className="space-y-4">
                <p className="text-sm leading-relaxed whitespace-pre-wrap">{notif.message_content}</p>
                {!notif.read_status && (
                  <Button
                    variant="outline"
                    size="sm"
